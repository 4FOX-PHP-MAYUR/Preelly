...
import {
  Edit2, Trash2, Plus, X, Search, ChevronUp, ChevronDown, LayoutList, RefreshCw, RotateCcw,
} from 'lucide-react'
...
function AdminFormFieldsPage() {
  ...
  const resolveFilterCategoryId = (categoryId, categoryFilterId) => (
  ...
  const handleCategoryFilterChange = (categoryFilterId) => {
    setForm((prev) => {
      ...
      return { ...prev, categoryFilterId, filterId: '' }
    })
  ...
  const getFormFieldScopeLabel = (categoryId, categoryFilterId = '') => {
    const categoryName = getCategoryName(categoryId)
    if (!categoryFilterId) return categoryName
  ...
  const isDuplicateFieldName = (fieldName, categoryId, categoryFilterId = '', excludeId = null) => {
  ...
  useEffect(() => {
    ...
    searchDebounceRef.current = setTimeout(() => {
      ...
      fetchFormFields(1, { search: searchInput })
    }, SEARCH_DEBOUNCE_MS)
    return () => {
      if (searchDebounceRef.current) clearTimeout(searchDebounceRef.current)
    }
  ...
  const openEdit = (row) => {
    ...
    const keepFilter = filterId && row.filterId?.name
      ...
      : null
    fetchCategoryChildren(catId).then(() => {
      const filterCategoryId = resolveFilterCategoryId(catId, catFilterId)
      if (filterCategoryId) {
        fetchFiltersForCategory(filterCategoryId, keepFilter)
      }
    })
    setShowForm(true)
    setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50)
  ...
  const validate = () => {
    ...
    } else if (isDuplicateFieldName(form.fieldName, form.categoryId, form.categoryFilterId, editing?._id)) {
      const scopeLabel = getFormFieldScopeLabel(form.categoryId, form.categoryFilterId)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      closeForm()
      fetchFormFields(editing ? page : 1)
    } catch (err) {
    ...
    <div className="max-w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-4">
        <form onSubmit={handleSearchSubmit} className="flex gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              placeholder="Search by Field Title, Field Name, Category..."
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Field Type</label>
            <select
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Quick View</label>
            <select
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-gray-100">
          <p className="text-sm text-gray-500">
            <span className="font-medium text-gray-700">{totalRecords}</span> total records
            {hasActiveFilters && (
              <>
                {' · '}
                <span className="font-medium text-indigo-600">{total}</span> matching filter
                {total !== 1 ? 's' : ''}
              </>
            )}
          </p>
          {hasActiveFilters && (
            <button
              ...
              className="px-3 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center gap-1.5 transition"
            >
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                ...
                <span className="ml-1 text-gray-400 font-normal">— unique per category filter scope, auto-generated from title</span>
              </label>
              ...
              <p className="mt-1 text-xs text-gray-400">
                Only lowercase letters, numbers and underscores. The same name can be reused in different categories or category filters.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                />
                <p className="mt-1 text-xs text-gray-400">Registered: filters, categories, emirates</p>
              </div>
              <div>
            ...
            {/* Dynamic table configuration for option-based fields */}
            {form.tableName.trim() && (
          ...
          <div className="p-14 text-center">
            ...
            {hasActiveFilters ? (
              <button
                type="button"
                onClick={handleResetFilters}
                className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50 transition"
              >
