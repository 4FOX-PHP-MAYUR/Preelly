...
const fs = require('fs');
const { ffmpeg, configureFfmpeg, isFfmpegAvailable, VIDEOS_HLS_ROOT, resolveScreenshotSize } = require('../services/ffmpegConfig');
...
const compressVideo = (req, res, next) => {
  ...
  const extractScreenshots = async ({ input, timestamps, filenamePattern, size }) =>
    new Promise((resolve, reject) => {
      ffmpeg(input)
        .screenshots({
          timestamps,
  ...
  const run = async () => {
    ...
    // Extract cinematic marketplace screenshots (AI-guided when OpenAI is configured).
    const filenamePrefix = `${baseName}-angle`
    const screenshotSize = await resolveScreenshotSize(compressedPath)
    let mapped = []
    try {
      mapped = await captureAutoScreenshotsFromVideo({
        videoPath: compressedPath,
        outputDir: screenshotsDir,
        filenamePrefix,
      })
    } catch (e) {
      // Fallback: try extracting only 3 evenly spaced frames.
      try {
        const fallbackTimestamps = [0.25, 0.5, 0.75].map((r) => Math.max(0, durationSeconds * r))
        const fallbackPattern = `${baseName}-angle-%03d.jpg`
        await extractScreenshots({
          input: compressedPath,
          timestamps: fallbackTimestamps,
          filenamePattern: fallbackPattern,
          size: screenshotSize,
        })
        const extractedFiles = fs
          ...
          .sort()
        mapped = extractedFiles.slice(0, fallbackTimestamps.length).map((fn, idx) => ({
          url: `/uploads/videos/screenshots/${fn}`,
          timestamp: fallbackTimestamps[idx],
          source: 'auto',
          captureSource: 'fallback_even_spacing',
        }))
      } catch (fallbackErr) {
