...
const REELS_FEED_DEFAULT_LIMIT = 50
const REELS_FEED_MAX_LIMIT = 100

function normalizeSortBy (sortBy) {
  const value = String(sortBy || 'random').toLowerCase().trim()
  if (value === 'newest' || value === 'recent') return 'latest'
  return value
}
...
function parseFeedParams (rawQuery) {
  ...
  const limit = Math.min(
    REELS_FEED_MAX_LIMIT,
    Math.max(1, Math.floor(Number(rawQuery.limit)) || REELS_FEED_DEFAULT_LIMIT)
  )
  const categoryId =
  ...
  const sortBy = normalizeSortBy(typeof rawQuery.sortBy === 'string' ? rawQuery.sortBy : 'random')
...
function applyVideoFilter (query) {
  const videoClause = {
    $or: [
      ...
      { 'videoStream.hlsUrl': { $exists: true, $nin: [null, ''] } },
      { 'videoStream.masterPlaylistUrl': { $exists: true, $nin: [null, ''] } },
      { 'videoStream.mp4Url': { $exists: true, $nin: [null, ''] } },
    ],
  }

  if (Array.isArray(query.$and)) {
    query.$and.push(videoClause)
  } else {
    query.$and = [videoClause]
  }

  return query
}

function applyCursorToMatch (match, decodedCursor) {
  ...
  if (Array.isArray(match.$and)) {
    match.$and.push(cursorClause)
  } else {
    match.$and = [cursorClause]
  }
...
async function createFeedResponse ({ posts, nextCursor, hasMore, feedType, page = 1, limit = REELS_FEED_DEFAULT_LIMIT }) {
  const enrichedPosts = await enrichReelsProducts(Array.isArray(posts) ? posts : [])
  return {
    feedType,
    posts: enrichedPosts,
    page,
...
function buildFeedAggregationStages ({ userObjectId }) {
  return [
    {
      $lookup: {
        from: 'users',
...
async function fetchRandomVideoFeed ({ userObjectId, params, savedProductIdsSet, feedType = 'trending' }) {
  ...
  if (excludeIdStrings.length > 0) {
    match._id = {
      $nin: excludeIdStrings
  ...
  try {
    posts = await Product.aggregate([
      ...
      ...buildFeedAggregationStages({ userObjectId }),
    ])
  } catch {
    const candidates = await Product.aggregate([
      { $match: match },
      { $limit: Math.min(sampleSize * 10, 500) },
      ...buildFeedAggregationStages({ userObjectId }),
    ])
    ...
    posts = candidates.slice(0, sampleSize)
  }
  ...
  const newExcludeIds = [
    ...excludeIdStrings,
    ...visiblePosts.map((post) => String(post._id)),
  ].slice(-1000)

  const hasMore = newExcludeIds.length < total && visiblePosts.length === sampleSize
  const nextCursor = hasMore ? encodeCursor({ excludeIds: newExcludeIds }) : null
...
async function fetchReelsForUser ({ userId, userObjectId, savedProductIdsSet, params }) {
  ...
  const enrichedProducts = await enrichReelsProducts(productsWithSaved)
  const reels = enrichedProducts.map((p) => ({
    ...p,
