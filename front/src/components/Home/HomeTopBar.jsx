...
/**
 * Top bar for the reels-style home page: search + quick-access icons + profile.
 * Kept as its own file (rather than inlined in HomePage) since it's a distinct
 * header variant from the shared site Header.
 */
function HomeTopBar({ mobileMenuOpen, onToggleMobileMenu }) {
  ...
  return (
    <div className="border-b border-slate-200 p-3 sm:p-5 lg:col-span-2 lg:grid lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-6">
      ...
      <div className="mt-3 hidden items-center gap-1 sm:flex lg:mt-0 lg:gap-2">
        <TopBarIcon to={isAuthenticated ? '/chat' : '/login'} label="Chat" Icon={MessageCircle} badge={unreadChatCount} />
        <TopBarIcon to="/search" label="My Search" Icon={SearchIcon} />
        <TopBarIcon to={isAuthenticated ? '/dashboard/listings' : '/login'} label="My Ads" Icon={FileText} />
        <TopBarIcon to={isAuthenticated ? '/dashboard/notifications' : '/login'} label="Notification" Icon={Bell} />
          ...
          >
            ...
            >
              <span className="relative flex-shrink-0">
                ...
                {isUserVerified(user) && (
                  <CheckCircle2 className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-white text-brand" />
                )}
              </span>
