...
import { FEATURED_LISTINGS_QUERY, FEATURED_RAIL_LIMIT } from '@shared/utils/featuredListings'
...
function ProductDetailPage({ adminMode = false }) {
  ...
  useEffect(() => {
    ...
    if (adminMode) return
    productService
      .getProducts({ ...FEATURED_LISTINGS_QUERY, limit: FEATURED_RAIL_LIMIT })
  ...
  if (!currentProduct) {
    return (
      <CategoryBrowseLayout variant="listing" layoutPreset="detail" adminMode={adminMode} showTrending={false} showMessages={false}>
