...
function withDeviceIdHeaders(extraHeaders = {}) {
  return {
    ...extraHeaders,
...
export const authService = {
  ...
  // Sign in with Apple — WEB endpoint. Separate from the mobile app's
  // POST /auth/apple: its own route, its own APPLE_WEB_* config on the server, and
  // it only accepts a token minted for the web Services ID. `user` carries the
  // display-only name hints Apple sends on a first authorization.
  appleWebLogin: ({ identityToken, authorizationCode, user }) =>
    api.post('/auth/apple/web', { identityToken, authorizationCode, user }),
...
export const videoService = {
  ...
  autoCaptureScreenshots: (videoFile) => {
    const formData = new FormData()
    formData.append('video', videoFile)
    return api.post('/ai/auto-capture-screenshots', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 300000,
    })
  },
...
// Global search (v1 web) — history tracked server-side by device-id + optional JWT
export const globalSearchService = {
  search: (params, config = {}) =>
    api.get('/v1/web/search', {
      ...config,
      params,
      headers: withDeviceIdHeaders(config.headers),
    }),
  getRecent: (config = {}) =>
    api.get('/v1/web/search/recent', {
      ...config,
      headers: withDeviceIdHeaders(config.headers),
    }),
  clearRecent: (config = {}) =>
    api.delete('/v1/web/search/recent', {
      ...config,
      headers: withDeviceIdHeaders(config.headers),
    }),
  getPopular: (params = {}, config = {}) =>
    api.get('/v1/web/search/popular', {
      ...config,
      params,
      headers: withDeviceIdHeaders(config.headers),
    }),
  getSuggestions: (params, config = {}) =>
    api.get('/v1/web/search/suggestions', {
      ...config,
      params,
      headers: withDeviceIdHeaders(config.headers),
    }),
