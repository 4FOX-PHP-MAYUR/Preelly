...
export default function VehicleFiltersBar({
  ...
  return (
    <div className="w-full bg-gray-100 border border-gray-200 rounded-2xl shadow-sm px-3 sm:px-4 py-3 flex flex-wrap items-end gap-2 sm:gap-0">
      {/* City */}
      <div className="flex flex-col min-w-[100px] sm:min-w-[120px] flex-1 sm:flex-initial sm:border-r sm:border-gray-300 sm:pr-3">
      ...
      {/* Make and Model */}
      <div className="flex flex-col flex-[2] min-w-[140px] sm:min-w-[200px] sm:border-r sm:border-gray-300 sm:px-3">
        <label className="text-xs font-medium text-gray-600 mb-1">Make And Model</label>
        <input
          type="text"
          aria-label="Search make, model, or trim"
          ...
          value={makeModel}
          onChange={(e) => onChangeMakeModel?.(e.target.value)}
        />
      </div>

      {/* Price Range */}
      <div className="flex flex-col min-w-[90px] sm:min-w-[110px] sm:border-r sm:border-gray-300 sm:px-3">
      ...
      <div className="ml-auto flex items-end sm:pl-2">
        ...
        <span className="hidden lg:inline text-xs text-gray-500 ml-1 self-center">Regional Specs, Keywords...</span>
      </div>
    </div>
  )
}
