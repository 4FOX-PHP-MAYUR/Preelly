...
import { Bell, Bookmark, Briefcase, Building2, Car, CheckCircle2, ChevronDown, ChevronRight, LayoutDashboard, LayoutGrid, LogOut, Menu, MessageCircle, Plus, Search, Settings, Shield, Shirt, Smartphone, Sofa, User, X } from 'lucide-react'
...
import { formatPrice, getCategoryImageUrl, getMediaUrl, isUserVerified, truncate } from '../utils/helpers'
import { getLocalReelsIndex, getReelsStorageKey, getSavedReelIndex, saveReelIndex } from '../utils/reelsProgress'
...
function HomePage() {
  ...
  const buildFeedParams = ({ cursor = null, refresh = false, targetFeedType = activeTab } = {}) => {
    const params = { limit: 10, feedType: targetFeedType, refresh }
        ...
        <div className="border-b border-slate-200 p-3 sm:p-5 lg:col-span-2 lg:grid lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-6">
          ...
          <form onSubmit={handleSearchSubmit} className="relative min-w-0">
            ...
            <input
              ...
              className="h-10 sm:h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-10 sm:pl-11 pr-4 text-sm text-slate-700 outline-none transition focus:border-primary-400 focus:bg-white"
            />
          </form>
