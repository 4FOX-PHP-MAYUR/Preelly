...
function ReelsFeedPage() {
  ...
  useEffect(() => {
    ...
    const params = { limit: 10, feedType: reelsTab, refresh: true }
    const query = new URLSearchParams(location.search || '')
    const selectedCategoryFromQuery = query.get('category')
  ...
  const loadMore = () => {
    ...
    const params = { limit: 10, cursor: nextCursor, feedType: reelsTab }
    const query = new URLSearchParams(location.search || '')
    const selectedCategoryFromQuery = query.get('category')
            ...
            <div className="w-full max-w-[640px]">
              ...
              <ReelsFeed
                ...
                onRefreshFeed={() => {
                  ...
                  if (activeSearch.trim()) params.search = activeSearch.trim()
                  if (advancedFilters.location) params.location = advancedFilters.location
                  if (advancedFilters.minPrice != null && advancedFilters.minPrice !== '') params.minPrice = Number(advancedFilters.minPrice)
                  if (advancedFilters.maxPrice != null && advancedFilters.maxPrice !== '') params.maxPrice = Number(advancedFilters.maxPrice)
                  if (advancedFilters.sortBy && advancedFilters.sortBy !== 'newest') params.sortBy = advancedFilters.sortBy
                  dispatch(fetchFeedPage(params))
                }}
              />
            </div>
